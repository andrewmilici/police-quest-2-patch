...
(procedure (FindFreeBooth &tmp temp0)
	
	(for ((= temp0 0))
		(< temp0 5)
		((++ temp0))
		;(Printf {temp0 %u} temp0)
		(if (not [shooter temp0])
			(return temp0)
;			(Printf {Shooter %u is NOT set} temp0)
		;else
			;(Printf {Shooter %u is set} temp0)
		)		
	)	
...
(instance rm10Script of Script
	...
	(method (handleEvent event &tmp currentX currentY newX newY)
		(switch (event type?)
			(saidEvent
				(cond 
					((Said 'look>')
					...
					((Said 'help/9mm')
						...
						(if (== (ego onControl: 1) cLMAGENTA)
							(if (ego has: iHandGun)
								(if (not (Btst fKenFixedGun))
									
									;(Printf {Current %u %u} (attendant2 x:) (attendant2 y:))
									;(= newX (GetNumber {New X}))
									;(= newY (GetNumber {New Y}))
									;(attendant2
									;	x: newX
									;	y: newY)
									...
									(self setScript: attendantScript)
...
(instance attendantScript of Script
	(method (changeState newState)
		;(Printf {changeState %u} newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(= freeBoothIndex (FindFreeBooth))
				(= attendantShooterX (- 207 (* freeBoothIndex 28)))
				(attendant x: -161 y: 156)
				(attendant2 x: 161 y: 156)
				(attendant2	setMotion: MoveTo 245 153 self)
			)
			(1
				(attendant2	setMotion: MoveTo 245 143 self)
			)
			(2
				(theDoor setMotion: MoveTo 270 137 self)
			)
			(3
				(attendant2	setMotion: MoveTo 244 130 self)
			)
			(4
				(theDoor setMotion: MoveTo 242 137 self)
			)
			(5
				...
				(attendant2	setMotion: MoveTo attendantShooterX 130 self)
			)
			(6
				(attendant2 
					x: -200
				...
				((= boothAttendantBody (Prop new:))
						posn: attendantShooterX 127
						setPri: 9
						loop: 1
						view: 73
						cycleSpeed: 2
				...
				;((= boothAttendantHead (View new:)) ;random head
					;	posn: attendantShooterX 98
					;	setPri: 9
					;	loop: 0
					;	cel: 2
					;	view: 73
			...
			(7 
				;(boothAttendantBody x: -200)
				;(boothAttendantHead x: -200)
				(boothAttendantHead dispose:)
				(boothAttendantBody dispose:)
				

				(attendant2 x: attendantShooterX y: 130)
				(attendant2	setMotion: MoveTo 244 130 self)
			)
			(8 
				(theDoor setMotion: MoveTo 270 137 self)
			...
			(11
				(attendant2	setMotion: MoveTo 242 156 self)
				
			)
			(12
				(attendant2	setMotion: MoveTo 161 156 self)
			)
			(13
				(attendant2 x: -245 153)
				(attendant x: 161 y: 156)	
				(= seconds 3)
			)
			(14
