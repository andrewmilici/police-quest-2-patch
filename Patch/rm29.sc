...
(instance rm29 of Room
	...
	(method (init &tmp temp0)
		...
		(= local9 44)
		;(= local15 30)
		(= local15 (if (== marieTime 1) 30 else 37))
		...
		((= newAutoDoor (AutoDoor new:))
			entranceTo: (if (== marieTime 1) 29 else 37)
			;entranceTo: 37
			facingLoop: 3
			view: 265
			posn: 186 105
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HandsOn)
				(if (== currentCar 13)
					(keith setMotion: MoveTo 185 156 self)
				)
