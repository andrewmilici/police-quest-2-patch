...
;;;		)
	...
	(method (handleEvent event &tmp index [temp1 53])
		...
		(switch (event type?)
			(saidEvent
				(cond 
					((and (Said 'kiss/angel>') (Said '/death')) ;"kiss angel of death" for debug
						(event claimed: TRUE)
						(if (^= debugging TRUE)
							(curRoom setLocales: DEBUG)
						)
					...
					((Said 'remove/cloth')
						...
						(if 
							...
							(theGame setScript: (ScriptID 11)) ; RemoveClothesScript
						else
								(Print 0 46)
							
						)
						;(Printf {x %u y %u} (ego x?) (ego y?))
